...
def load_dataset(input_dir, num, pt_background_cut, pt_signal_cut, eta_cut, true_edges, noise):
    if input_dir is not None:
        ...
        print("Events loaded!")
        loaded_events = select_data(loaded_events, pt_background_cut, pt_signal_cut, eta_cut, true_edges, noise)
        print("Events processed!")
        return loaded_events
    else:
        return None

def select_data(events, pt_background_cut, pt_signal_cut, eta_cut, true_edges, noise):
    ...
    if (pt_background_cut > 0):
        for i, event in enumerate(events):   
            ...
            pt_mask = (event.pt[event.edge_index] > pt_background_cut).all(0)
            eta_mask = ( (eta[event.edge_index] > - eta_cut) & (eta[event.edge_index] < eta_cut) ).all(0)
            
            edge_mask = pt_mask & eta_mask
